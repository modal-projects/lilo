...
def deploy(desired, *, refresh_trainers=(), refresh_inference=()):
    """Deploy missing workers, then publish the frontend's configuration."""
    if sys.version_info[:2] != (3, 12):
        raise ValueError(
            "Python deployment requires Python 3.12 to match the serialized GPU runtime images"
        )
    settings = desired[0].platform
    environment = settings["modal"]["environment"]
    previous = [
        DeploymentRecord.model_validate(row)
        for row in deployed_manifest(settings["frontend"], environment)
    ]
    ...
    for row in desired:
        for role, app_name in (
            ...
            try:
                ...
                subprocess.run(
                    [*command, "-m", "lilo.providers.modal.deployment_worker_app"],
                    check=True,
                    env=worker_env,
                )
    subprocess.run(
        [*command, "-m", "lilo.providers.modal.app"],
        check=True,
        env={
            **os.environ,
            MANIFEST_ENV: json.dumps([row.model_dump(mode="json") for row in manifest]),
