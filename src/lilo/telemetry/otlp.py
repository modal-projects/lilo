...
def provider():
    ...
    try:
        ...
        if protocol != "http/protobuf":
            raise ValueError("Lilo sampling traces require http/protobuf")
        ...
        logger.warning("Lilo OTLP initialization failed; tracing disabled")
        return None
...
def sample_trace(task, stats):
    ...
    span = p.get_tracer("lilo.sampling").start_span(
        "lilo.sample",
        context=Context(),
    ...
    _attributes(
        span,
        {
            "lilo.request_id": task.get("request_id"),
            "lilo.model_id": task.get("model_id")
            or "base:" + str(task.get("engine_definition_id")),
            "lilo.base_model": task.get("base_model"),
            "lilo.num_samples": count,
            "lilo.version_requested": task.get("publish_version"),
            "lilo.latest": bool(task.get("latest")),
            "lilo.start_boundary": "accepted" if start == accepted else "worker",
        },
    )
    state = {
        "span": span,
        "tracer": p.get_tracer("lilo.sampling"),
        "tags": tags,
        "attempts": 0,
        "retries": 0,
    }
    ...
    try:
        ...
        _attributes(
            span,
            {
                "lilo.input_tokens": stats.get("prompt_tokens"),
                "lilo.output_tokens": stats.get("generated_tokens"),
                "lilo.attempt_count": state["attempts"],
                "lilo.retry_count": state["retries"],
            },
        )
        ...
        if count == 1:
            _attributes(
                span,
                {
                    "lilo.version_served_start": stats.get("version_served_start"),
                    "lilo.version_served_end": stats.get("version_served_end"),
                },
            )
        span.end()
...
def record_attempt(event):
    ...
    _attributes(
        ...
        {
            **state["tags"],
            "lilo.request_id": attrs.get("request_id"),
            "lilo.attempt_id": attrs.get("attempt_id"),
            "lilo.sequence_index": attrs.get("sequence_index"),
            "lilo.attempt_number": attrs.get("attempt_number"),
            "lilo.input_tokens": attrs.get("prompt_tokens"),
            "lilo.output_tokens": attrs.get("generated_tokens"),
