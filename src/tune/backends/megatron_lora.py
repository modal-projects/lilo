...
from tune.engine.spmd import DistributedExecutor, initialize_distributed_runtime
from tune.telemetry.backend import measured
...
class LoraMegatronBackend(Backend):
        ...
        *,
        ...
        self._checkpoint_captures[snapshot_id] = {
            ...
            "metadata": {
                ...
                "engine_definition_id": os.environ["TUNE_DEFINITION_ID"],
    ...
    def capture_sampler_snapshot(
        ...
        bulletin_root = os.environ["TUNE_BULLETIN_ROOT"]
        state = self.jobs[model_id]
        ...
        snapshot = capture_adapter_snapshot(
            ...
            bulletin_volume=os.environ["TUNE_BULLETIN_VOLUME"],
        )
...
def build_executor() -> DistributedExecutor:
    ...
    config, checkpoint_dir = parse_backend_config(
        json.loads(os.environ["TUNE_BACKEND_CONFIG"])
    )
    backend = LoraMegatronBackend(
        config,
        checkpoint_dir=checkpoint_dir,
        base_model=os.environ["TUNE_BASE_MODEL"],
