...
def create_engine_app(server: EngineApi, *, token: str | None = None) -> FastAPI:
    ...
    from tune.telemetry.trainer import CommandMiddleware
    ...
    def __init__(
        ...
        *,
        ...
        async def inject_context(request):
            from tune.telemetry.trainer import headers as trace_headers
        ...
        async def read_context(response):
            ...
            from tune.telemetry.trainer import accepted_root

            canonical = response.headers.get("x-tune-command-traceparent")
            if canonical:
                context = extract({"traceparent": canonical})
                try:
                    raw = json.loads(response.headers.get("x-tune-command-tags", "{}"))
                    tags = {
                        k: v
                    ...
                    tags = {}
                accepted_root.set(set_value("tune.command.tags", tags, context))
