...
from tune.telemetry import backend as telemetry
from tune.telemetry.otlp import provider
...
def create_backend_app(executor: Executor) -> FastAPI:
    ...
    async def collect_measurements(request, call_next):
        with telemetry.recording(request.headers.get("x-tune-telemetry") == "1"):
    ...
    async def _post(self, path: str, body: dict) -> object:
        ...
        try:
            response = await self.http.post(
                path,
                json=body,
                headers={"x-tune-telemetry": "1"} if enabled else {},
            )
