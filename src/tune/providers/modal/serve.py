...
from tune.engine.backend_http import HttpBackendClient
from tune.engine.http import create_engine_app
...
async def _kick_trainer_reconciler(definition_id: str) -> None:
    ...
    async def spawn(delay_seconds: float) -> str:
        function = modal.Function.from_name("tune", "trainer_reconciler")
...
async def serve_engine(
    ...
    *,
    ...
    try:
        ...
        from tune.telemetry.trainer import TrainerTelemetry

        telemetry = TrainerTelemetry(
            instance_id, definition_id, record.boot_id,
            scoped=bool(os.environ.get("TUNE_SCOPED_REGISTRY")),
        )
...
def run_engine_with_backend(
    ...
    *,
    ...
    backend = subprocess.Popen(
        [*launcher, "tune.engine.backend_http", executor_reference, str(port)],
        env=env,
