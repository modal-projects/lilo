...
APP_NAME = os.environ["TUNE_FFT_POOL_APP_NAME"]
DEFINITION_ID = os.environ["TUNE_FFT_POOL_DEFINITION_ID"]
MODEL_ID = os.environ["TUNE_FFT_POOL_MODEL_ID"]
LATEST = os.environ["TUNE_FFT_POOL_LATEST"] == "1"
VERSION = int(os.environ["TUNE_FFT_POOL_VERSION"])
definition = importlib.import_module(
    f"tune.providers.modal.definitions.{DEFINITION_ID}"
)
...
def _pool_setting(name: str, default: int | None) -> int | None:
    value = os.environ.get(f"TUNE_FFT_POOL_{name}")
...
pool_secret = modal.Secret.from_dict(
    {
        ...
        if key.startswith("TUNE_FFT_POOL_")
    }
)
