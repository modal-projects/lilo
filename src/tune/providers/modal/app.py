...
@app.function(
    ...
    timeout=60 * 60,
    retries=2,
    secrets=[proxy_secret, modal.Secret.from_name("tune-api")],
)
@modal.concurrent(max_inputs=128)
async def execute_sample(task: dict) -> dict:
    from tune.telemetry.otlp import sample_trace
...
@app.function(
    ...
    secrets=[modal.Secret.from_name("tune-api", required_keys=["TINKER_API_KEY"])],
)
@modal.concurrent(max_inputs=128)
@modal.asgi_app(requires_proxy_auth=False)
def server():
    from tune.control_plane import create_control_plane_app
