    ...
    *,
    ...
    for traj_group in trajectory_groups_P:
        ...
        for traj in traj_group.trajectories_G:
            ...
            lens_G.append(max(traj_tokens, 1))
            total_tokens += traj_tokens
            num_trajectories += 1
        response_lens_P.append(torch.tensor(lens_G, dtype=torch.float32))
    ...
    if adv_weighting == "sample_mean" and total_tokens > 0:
        # Miles-style per-sample mean: token t of datum i gets weight
        # A_i * T / (n * len_i); the summed weight over the batch is unchanged.
        advantages_P = [
            advantages * (total_tokens / (num_trajectories * lens_G))
            for advantages, lens_G in zip(advantages_P, response_lens_P, strict=True)
        ]
    elif adv_weighting != "token_mean":
        raise ValueError(f"unknown adv_weighting: {adv_weighting}")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="token_mean",
        help="token_mean: A_i/T (czzwpqau); sample_mean: A_i*T/(n*len_i) (Miles-style)",
    )
