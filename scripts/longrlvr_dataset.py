...
def _chunk_blocks(content: str) -> list[tuple[int, str]]:
    return [(int(index), body) for index, body in _CHUNK_BLOCK_PATTERN.findall(content)]


# `\n<CHUNK_12345>` plus its closing tag, which the per-chunk body counts miss.
_CHUNK_WRAPPER_TOKENS = 16


def _pad_prompt_with_distractors(
    prompt: list[renderers.Message],
    distractors: Sequence[tuple[str, int]],
    *,
    budget_tokens: int,
) -> list[renderers.Message]:
    """Append unrelated chunks after the document so the prompt reaches 256k.

    Chunk ids continue past the document's last id, so ``ref_chunks`` and the
    reward stay exactly as they are in the source row.
    """
    messages = [dict(message) for message in prompt]
    content = str(messages[-1].get("content", ""))
    blocks = _chunk_blocks(content)
    if not blocks or budget_tokens <= 0:
        return messages
    tail_marker = f"</CHUNK_{blocks[-1][0]}>"
    split_at = content.rindex(tail_marker) + len(tail_marker)
    next_index = max(index for index, _ in blocks) + 1

    padding: list[str] = []
    remaining = budget_tokens
    for body, body_tokens in distractors:
        chunk_tokens = body_tokens + _CHUNK_WRAPPER_TOKENS
        if chunk_tokens > remaining:
            continue
        padding.append(f"\n<CHUNK_{next_index}>{body}</CHUNK_{next_index}>")
        next_index += 1
        remaining -= chunk_tokens
        if remaining <= 0:
            break
    messages[-1]["content"] = content[:split_at] + "".join(padding) + content[split_at:]
    return messages
...
class LongRLVRDataset(RLDataset):
        ...
        *,
        ...
        self.rows = (
            ...
            if prompt_file is not None
                ...
                seed=seed,
                pad_to_tokens=min(pad_to_tokens, max_prompt_tokens),
            )
        )
    ...
    def _prompt_tokens(self, prompt: list[renderers.Message]) -> int:
        ...
        return len(self.tokenizer(rendered, add_special_tokens=False)["input_ids"])
        ...
        *,
        ...
        for row in dataset:
            ...
            if len(rows) == num_groups:
                # Extra rows only donate distractor chunks for prompt padding.
                for _, body in _chunk_blocks(
                    str(prompt[-1].get("content", "")) if prompt else ""
                ):
                    body_tokens = len(
                        self.tokenizer(body, add_special_tokens=False)["input_ids"]
                    )
                    distractors.append((body, body_tokens))
                    distractor_tokens += body_tokens
                if distractor_tokens >= 2 * pad_to_tokens:
                    break
                continue
            ...
            if len(rows) == num_groups and pad_to_tokens <= 0:
                break
        if len(rows) < num_groups:
            raise RuntimeError(
        ...
        if pad_to_tokens > 0:
            self._pad_rows(rows, distractors, pad_to_tokens=pad_to_tokens)
        self.prompt_token_lengths = [int(row["prompt_tokens"]) for row in rows]
        return rows
    ...
    def _pad_rows(
        ...
        *,
        ...
        """Grow every prompt towards ``pad_to_tokens`` with unrelated chunks.

        Each row starts at a different offset in the distractor pool so the
        padding is deterministic without being identical across rows.
        """
        if not distractors:
            raise RuntimeError("no distractor chunks available for prompt padding")
        for index, row in enumerate(rows):
            offset = (index * 17) % len(distractors)
            rotated = list(distractors[offset:]) + list(distractors[:offset])
            padded = _pad_prompt_with_distractors(
                row["prompt"],
                rotated,
                budget_tokens=pad_to_tokens - int(row["prompt_tokens"]),
            )
            row["prompt"] = padded
            row["prompt_tokens"] = self._prompt_tokens(padded)
            print(
                f"[longrlvr] row {index}: prompt padded to "
                f"{row['prompt_tokens']} tokens (target {pad_to_tokens})",
                flush=True,
            )
