...
These runs exercise independent LoRA clients sharing a Miles trainer and rollout
...
### Six asynchronous clients on upstream Miles

A separate run uses the ordinary, nondeterministic Miles backend with the same
...
This regenerates all four PNGs without Modal or W&B access. It reproduces the
