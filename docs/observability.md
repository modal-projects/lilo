...
| Local LoRA admission retry | `lilo.inference.admission_retries` when a zero-token queue-full response was retried inside the replica; these retries are separate from sampling HTTP attempts |
